...
class Server:
    ...
    def handle_client_send(self, conn, addr, kill_event, player_id):
    ...
    def handle_client_recv(self, conn, addr, kill_event, player_id):
    ...
    def run(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ...
                thread1 = threading.Thread(target=self.handle_client_recv, args=(conn, addr, self.kill_event, player_id))
                thread1.start()
                thread2 = threading.Thread(target=self.handle_client_send, args=(conn, addr, self.kill_event, player_id))
                thread2.start()
